...
export class QueueService {
  ...
  async enqueue(prompt: string, requestedBy: string): Promise<ExecutionJob> {
    ...
    try {
      ...
      this.logger.log(`Job enqueued: ${job.id.slice(0, 8)} by ${requestedBy}`);
      return job;
    } finally {
      this.lock.release();
    }
  ...
  async updateJob(
    jobId: string,
    updates: Partial<Pick<ExecutionJob, 'status' | 'pid' | 'result' | 'completedAt' | 'error' | 'cancelledAt'>>,
  ): Promise<void> {
    await this.lock.acquire();
    try {
      const queue = this.readQueue();
